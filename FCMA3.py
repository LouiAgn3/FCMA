...
def main():
    ...
    # --- MODIFIED: Using the new disjoint partitioning logic ---
    ...
    # Step 1: For each class, find clients that need it
    class_to_clients = {c: [] for c in range(10)}
    for client_id in range(NUM_CLIENTS):
        class1 = client_id % 10
        class2 = (client_id + 1) % 10
        class_to_clients[class1].append(client_id)
        class_to_clients[class2].append(client_id)
    
    # Step 2: For each class, split its indices disjointly among the clients
    for class_id in range(10):
        client_list = class_to_clients[class_id]
        if not client_list:
            continue
        
        # Shuffle indices for randomness before splitting
        np.random.shuffle(indices_by_class[class_id])
        
        # Split the indices for this class as evenly as possible among the clients
        disjoint_split = np.array_split(indices_by_class[class_id], len(client_list))
        
        for i, client_id in enumerate(client_list):
            client_data_indices[client_id].extend(disjoint_split[i])
    ...
    print(f"Created {len(client_dataloaders)} non-IID client dataloaders successfully (disjoint partitions).")
    ...
    print("\nStarting Federated Training (Fed-CMA)...")
    criterion = nn.CrossEntropyLoss()
