...
def partition_data_non_iid_dirichlet(dataset, num_clients, alpha=0.5):
    ...
    # Convert tensor to numpy array correctly to avoid DeprecationWarning.
    try:
        labels = dataset.targets.numpy()
    except AttributeError:
        labels = np.array([sample[1] for sample in dataset]) # Slower fallback
    ...
    for i in range(num_clients):
        ...
        # Only create a DataLoader if the client has been assigned data.
        if len(indices) > 0:
...
def main():
    ...
    # --- BUG FIX: Use the actual number of clients, not the initial number ---
    ...
    # --- BUG FIX: Use actual_num_clients in loop range ---
    for i in range(min(10, actual_num_clients)):
    ...
    if not initial_updates:
        print("Not enough clients to perform PCA. Exiting.")
        return
    ...
    # --- BUG FIX: Use actual_num_clients for assignments ---
    client_cluster_assignments = np.random.randint(0, NUM_CLUSTERS, actual_num_clients)
    # --- BUG FIX: Use actual_num_clients to initialize local models ---
    ...
    for round_num in tqdm(range(NUM_ROUNDS), desc="Federated Rounds"):
        ...
        if round_num > 0 and round_num % RECLUSTERING_INTERVAL == 0:
            # --- BUG FIX: Use actual_num_clients for matrix and loop sizes ---
            ...
            model_updates = []
            # --- BUG FIX: Use actual_num_clients in loop ---
            for i in range(actual_num_clients):
