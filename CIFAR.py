...
# MODIFICATION: This function now splits each client's data into train and test sets.
def partition_data_non_iid_dirichlet(dataset, num_clients, alpha=0.1, train_split_ratio=0.8):
    ...
    for inds in client_indices:
        if inds:
            ...
            else: # Ensure every client has a loader, even if empty
...
def calculate_s_data(client_dataloaders):
    ...
    for loader in client_dataloaders:
        ...
        labels = []
        # Handle Subset objects to get to the underlying labels
...
def evaluate(model, test_loader):
    ...
    correct = 0
    total = 0
    if not test_loader.dataset: return 0.0 # Return 0 accuracy for empty loaders
    with torch.no_grad():
...
def main():
    ...
    # MODIFICATION: Capture both train and test loaders for each client
    ...
    actual_num_clients = len(client_trainloaders)
    ...
    for i in range(num_pca_samples):
        if not client_trainloaders[i].dataset: continue # Skip empty clients
        client_model = copy.deepcopy(temp_model)
        optimizer = optim.SGD(client_model.parameters(), lr=LEARNING_RATE)
        criterion = nn.CrossEntropyLoss()
        client_model.train()
        for data, target in client_trainloaders[i]:
            data, target = data.to(DEVICE), target.to(DEVICE)
            optimizer.zero_grad()
    ...
    # MODIFICATION: Lists to track both accuracy types
    cluster_accuracies = []
    ...
    for round_num in tqdm(range(NUM_ROUNDS), desc="Federated Rounds"):
        ...
        for client_id in range(actual_num_clients):
            # Skip clients with no training data
            if not client_trainloaders[client_id].dataset:
                current_local_models.append(copy.deepcopy(local_models[client_id])) # Keep old model
                continue
        ...
        # MODIFICATION: Calculate personalized accuracy for the round
    ...
    # MODIFICATION: Plot both accuracy metrics
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('fed-cma-accuracies-cifar10-non-iid.png')
    plt.close()
    print("Plot saved to fed-cma-accuracies-cifar10-non-iid.png")
