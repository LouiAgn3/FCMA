...
def partition_data_by_can_id(df, num_clients):
    ...
    # Debug: Check how many clients have data
    active_clients = 0
    for i, loader in enumerate(client_dataloaders):
        if hasattr(loader.dataset, 'labels') and len(loader.dataset.labels) > 0 and len(loader.dataset) > 0:
            active_clients += 1
            print(f"Client {i}: {len(loader.dataset)} samples")
        else:
            print(f"Client {i}: Empty dataset")
